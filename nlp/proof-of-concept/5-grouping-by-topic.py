...
def calculate_similarities(embeddings):
    similarities = {}
    for feed1, embeddings1 in embeddings.items():
        for feed2, embeddings2 in embeddings.items():
            if feed1 == feed2:
                continue

            if f"{feed1},{feed2}" in similarities:
                continue

            # cosine similarity
            similarities[f"{feed1},{feed2}"] = model.similarity(embeddings1, embeddings2)

    return similarities

embeddings = calculate_embeddings(all_feeds)
similarities = calculate_similarities(embeddings)

THRESHOLD = 0.6
groups = []

def any_group_contains_either(article1, article2):
    for group_id, group in enumerate(groups):
        if article1 in group or article2 in group:
            return group_id

    return -1

def add_to_group_unless_present(group_id, article1, article2):
    existing_group = groups[group_id]

    if article1 not in existing_group:
        groups[group_id] = groups[group_id] + [article1]

    if article2 not in existing_group:
        groups[group_id] = groups[group_id] + [article2]

for feeds, scores in similarities.items():
    feed1, feed2 = feeds.split(',')

    for idx_i, sentence1 in enumerate(all_feeds[feed1]):
        for idx_j, sentence2 in enumerate(all_feeds[feed2]):
            if scores[idx_i][idx_j] >= THRESHOLD:
                found_group = any_group_contains_either(sentence1, sentence2)
                if found_group > -1:
                    add_to_group_unless_present(found_group, sentence1, sentence2)
                else:
                    groups = groups + [[sentence1, sentence2]]

# for each feed
# for each article
# if other articles are above similarity threshold:
#   if any of the lists already contain either article:
#       add to that group (unless already present)
#   else, add both to new group in lists of groups
